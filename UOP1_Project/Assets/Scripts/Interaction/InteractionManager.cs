...
public class InteractionManager : MonoBehaviour
{
	...
	void OnInteractionButtonPress()
	{
		...
		switch (_potentialInteraction)
		{
			...
			case InteractionType.Save:
				if (_currentInteractableObject != null)
				{
					_currentInteractableObject.GetComponent<SavePoint>().Save();
					Debug.Log("Save Event Raised");
				}
				break;
			default:
				break;
	...
	private void OnTriggerEnter(Collider other)
	{
		...
		{
			_potentialInteraction = InteractionType.Save;
			//Debug.Log("I triggered an Save!");
			DisplayInteractionUI();
		}
