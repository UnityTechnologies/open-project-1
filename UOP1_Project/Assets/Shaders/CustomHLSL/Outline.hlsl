...
void OutlineObject_float(float2 UV, float OutlineThickness, float DepthSensitivity, float NormalsSensitivity, float DepthNormalSensitivity, float DepthNormalThresholdScale, float3 ViewDir, out float Out)
{
	...
	uvSamples[3] = UV + float2(_CameraDepthTexture_TexelSize.x * screenBiasedTickness, -_CameraDepthTexture_TexelSize.y * screenBiasedTickness);
	uvSamples[4] = UV + float2(-_CameraDepthTexture_TexelSize.x * screenBiasedTickness, _CameraDepthTexture_TexelSize.y * screenBiasedTickness);
